...
local UiLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/ArcaneAw/UIS/main/Modded.lua"))()
...
StatsPicture.Name = "DhEnhancedAnti"
...
StatsHealthBackground.Name = "DhEnhancedAnti"
...
StatsHealthBar.Name = "DhEnhancedAnti"
...
local NewWindow = UiLib:CreateWindow("DhEnhanced Anti | 7rxp | .gg/s1lence", Vector2.new(500, 600), Enum.KeyCode.V)

-- Tabs

local MainTab = NewTab:CreateTab("Main")
local MiscTab = NewTab:CreateTab("Misc")
		...
		if TargBindEnabled then
			...
			if TargetAimbot.Notify then
				StarterGui:SetCore("SendNotification", {
					Title = "DhEnhancedAnti", 
					Text = "Untargetting: " .. TargetPlr.DisplayName, 
					Duration = 3
				})
			end
		else
	...
	if CameraAimbot.Enabled and Key.KeyCode == CameraAimbot.Keybind and not UserInputService:GetFocusedTextBox() then
		...
		if CamBindEnabled then
			...
			if CameraAimbot.Notify then
				StarterGui:SetCore("SendNotification", {
					Title = "DhEnhancedAnti", 
					Text = "Untargetting: " .. CamlockPlr.DisplayName, 
					Duration = 3
				})
			end
		else
